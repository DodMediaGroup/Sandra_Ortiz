...
class ProyectosController extends Controller
{
    ...
    public function actionAdmin()
    {
        ...
        $proyectos = Proyectos::model()->findAll(array('condition'=>'t.estado != 2', 'order'=>'t.id DESC'));
    ...
    public function actionCreate()
    {
        ...
        if(isset($_POST['Proyectos']))
        {
            ...
            if($model->validate(null, false)){
                ...
                if(!(MyMethods::isValidDate($_POST['Proyectos']['fecha'], 'd/m/Y'))){
                    $model->fecha = "";
                    $model->addError('fecha', 'El campo fecha no es una fecha valida!!!');
                    $errors = true;
                }
                else{
                    $fecha = @date_create(str_replace("/","-",trim($_POST['Proyectos']['fecha'])), new DateTimeZone('Europe/London'));
                ...
                if(($_FILES['image']['size'] > 0)){
                    ...
                    $uploadLogo = MyMethods::uploadImage($_FILES['image'], 150, 'proyectos/');
                    ...
                    if(!$uploadLogo['status']){
                        ...
                        MyMethods::resizeImage($server.'proyectos/', $model->imagen, 150, 150);
                    }
                }
            ...
            if($model->validate(null, false)){
                ...
                if(($_FILES['image']['size'] > 0)){
                    ...
                    if(!$uploadLogo['status']){
                        ...
                        if($currentImage != ""){
                            @unlink($_SERVER['DOCUMENT_ROOT'].Yii::app()->request->baseUrl."/images/proyectos/".$currentImage);
                            @unlink($_SERVER['DOCUMENT_ROOT'].Yii::app()->request->baseUrl."/images/proyectos/150x150/".$currentImage);
                        }
                    }
                }
     ...
     */
    public function actionDelete_proyecto($id)
    {
        $proyecto = $this->loadModel($id);
    ...
    {
        $model=Proyectos::model()->findByAttributes(array('id'=>$id), array('condition'=>'t.estado != 2'));
        if($model===null)
            throw new CHttpException(404,'The requested page does not exist.');
        return $model;
    }
