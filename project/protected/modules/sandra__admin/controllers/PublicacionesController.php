...
class PublicacionesController extends Controller
{
    ...
    public function actionAdmin()
    {
        ...
        $publicaciones = Publicaciones::model()->findAll(array('condition'=>'t.estado != 2', 'order'=>'t.id DESC'));
    ...
    public function actionCreate()
    {
        ...
        if(isset($_POST['Publicaciones']))
        {
            ...
            if($model->validate(null, false)){
                ...
                if(!(MyMethods::isValidDate($_POST['Publicaciones']['fecha'], 'd/m/Y'))){
                    $model->fecha = "";
                    $model->addError('fecha', 'El campo fecha no es una fecha valida!!!');
                    $errors = true;
                }
                else{
                    $fecha = @date_create(str_replace("/","-",trim($_POST['Publicaciones']['fecha'])), new DateTimeZone('Europe/London'));
                ...
                if(($_FILES['image']['size'] > 0)){
                    ...
                    $uploadLogo = MyMethods::uploadImage($_FILES['image'], 150, 'publicaciones/');
                    ...
                    if(!$uploadLogo['status']){
                        ...
                        MyMethods::resizeImage($server.'publicaciones/', $model->imagen, 150, 150);
                    }
                }
            ...
            if($model->validate(null, false)){
                ...
                if(($_FILES['image']['size'] > 0)){
                    ...
                    if(!$uploadLogo['status']){
                        ...
                        if($currentImage != ""){
                            @unlink($_SERVER['DOCUMENT_ROOT'].Yii::app()->request->baseUrl."/images/publicaciones/".$currentImage);
                            @unlink($_SERVER['DOCUMENT_ROOT'].Yii::app()->request->baseUrl."/images/publicaciones/150x150/".$currentImage);
                        }
                    }
                }
    ...
    {
        $model=Publicaciones::model()->findByAttributes(array('id'=>$id), array('condition'=>'t.estado != 2'));
        if($model===null)
            throw new CHttpException(404,'The requested page does not exist.');
        return $model;
    }
