...
class DebatesController extends Controller
{
    ...
    public function actionAdmin()
    {
        ...
        $debates = Debates::model()->findAll(array('condition'=>'t.estado != 2', 'order'=>'t.id DESC'));
    ...
    public function actionCreate()
    {
        ...
        if(isset($_POST['Debates']))
        {
            $errors = false;
            $model->attributes=$_POST['Debates'];
            $model->clearErrors();

            if($model->validate(null, false)){
                date_default_timezone_set('America/Bogota');

                if(!(MyMethods::isValidDate($_POST['Debates']['fecha'], 'd/m/Y'))){
                    $model->fecha = "";
                    $model->addError('fecha', 'El campo fecha no es una fecha valida!!!');
                    $errors = true;
                }
                else{
                    $fecha = @date_create(str_replace("/","-",trim($_POST['Debates']['fecha'])), new DateTimeZone('Europe/London'));
                ...
                if(($_FILES['image']['size'] > 0)){
                    ...
                    $uploadLogo = MyMethods::uploadImage($_FILES['image'], 150, 'debates/');
                    ...
                    if(!$uploadLogo['status']){
                        ...
                        MyMethods::resizeImage($server.'debates/', $model->imagen, 150, 150);
                    }
                }
                else{
            ...
            if($model->validate(null, false)){
                ...
                if(($_FILES['image']['size'] > 0)){
                    ...
                    if(!$uploadLogo['status']){
                        ...
                        if($currentImage != ""){
                            @unlink($_SERVER['DOCUMENT_ROOT'].Yii::app()->request->baseUrl."/images/debates/".$currentImage);
                            @unlink($_SERVER['DOCUMENT_ROOT'].Yii::app()->request->baseUrl."/images/debates/150x150/".$currentImage);
                        }
                    }
                }
    ...
    public function actionEstado($id){
        $debate = $this->loadModel($id);
        if($debate->estado == 1)
            $debate->estado = 0;
        else if($debate->estado == 0)
            $debate->estado = 1;
        else
     ...
     */
    public function actionDelete_debate($id)
    {
        $debate = $this->loadModel($id);
    ...
    {
        $model=Debates::model()->findByAttributes(array('id'=>$id), array('condition'=>'t.estado != 2'));
        if($model===null)
            throw new CHttpException(404,'The requested page does not exist.');
        return $model;
    }
