 ...
 * @property PublicacionesCategorias $categoria0
 */
class Publicaciones extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('titulo, categoria, fecha', 'required'),
			array('categoria, estado', 'numerical', 'integerOnly'=>true),
			array('titulo, fuente', 'length', 'max'=>255),
			array('contenido', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, titulo, categoria, fecha, contenido, fuente, estado', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			'categoria0' => array(self::BELONGS_TO, 'PublicacionesCategorias', 'categoria'),
		);
	}
